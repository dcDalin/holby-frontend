(function($) {
  $.fn.extend({
    paging: function(options) {
      ...
      options.totalPages = Math.ceil(
        options.currentData.length / options.perPage
      );
  ...
  function setupPager(pager, options) {
    ...
    $("#" + options.contentHolder).html(
      template(
        ...
        options.startPage,
        options.perPage,
        options.alwaysShowPager,
        options.informationToShow,
    ...
    $("#" + pager).on("keyup", ".search", function(e) {
      ...
      var newJson = refineJson(
        options.data,
        refine,
        options.informationToRefineBy
      );
  ...
  function createPaging(
    ...
    for (var i = 1; i < pages; i++) {
      html +=
        '<span class="page btn" data-value="' +
        (i + 1) +
        '">' +
        (i + 1) +
        "</span>";
    ...
    html +=
      '	<span class="next-pages btn" data-value="6">...</span>' +
      '<span class="next-page btn">Next</span>' +
      '<span class="last-page btn" data-value="' +
      pages +
      '">Last</span>' +
      "</div>";

    html +=
      '<div class="options" style="text-align: center; margin-bottom: 10px;">' +
      "<span>Show </span>" +
  ...
  function template(
    ...
    var last =
      page * itemsPerPage > jsonData.length
        ? jsonData.length
        : page * itemsPerPage;
    ...
    if (jsonData.length <= 0) {
      if (errorTemplate == "") {
        ...
        var error = $("#" + errorTemplate)
          .html()
          .format(["There are no Messages to display."]);
        html += error;
      }
    ...
    $("#" + pager + " .showing").html(
      "Showing " + first + " to " + last + " of " + jsonData.length + " total."
    );

    for (
      var i = page * itemsPerPage - itemsPerPage;
      i < page * itemsPerPage;
      i++
    ) {
  ...
  function refineJson(jsonToRefine, refineStatment, informationToRefineBy) {
    ...
    for (var i = 0; i < jsonToRefine.length; i++) {
      $.each(informationToRefineBy, function(key, value) {
        if (
          jsonToRefine[i][value] != null &&
          jsonToRefine[i][value]
            .toLowerCase()
            .indexOf(refineStatmentToLowerCase) >= 0
        ) {
